...
//Lrun.runL();
    ...
    //Участок кода формирования баз их заполнения из файла (основа работает)
    static void fillDB(String patch_file) throws IOException {
        ...
        Iterator<String> it_list_sheet = rwexel.get_list_sheet().iterator(); //забираем список листов в файле и строим итератор из них
        while (it_list_sheet.hasNext()) {
            String name_table = it_list_sheet.next(); // название таблицы как имя листа из файла
            int maxlencol;
            if (name_table.equals("DO1")) {
                 maxlencol = rwexel.getMaxcColumn(name_table); //количество столбцов методом вычисления в фукции
            }else  maxlencol = rwexel.getMaxcColumn(name_table); //количество столбцов методом вычисления в фукции
            ArrayList<String> name_collums = new ArrayList<>(rwexel.getDataNameTable(name_table)); // получаем массив столбцов и формируем от куда начинать считывать данные
            // так переопределим длину от куда тащим названия
            maxlencol = name_collums.size();
            workbase.createTable(name_table, maxlencol, name_collums); // передаем название таблицы и максимальное  - создание таблиц
            // берем данные с файла
            ArrayList<String[]> sheet_fromsheet_from;
            sheet_fromsheet_from = new ArrayList<>(rwexel.getDataCell(name_table, maxlencol)); // maxlencol не верное вычисление похоже на 3 меньше в DO1 чем должно быть
            Iterator<String[]> iter_sheet = sheet_fromsheet_from.iterator();
            while (iter_sheet.hasNext()) {
                String[] dataFromFile = iter_sheet.next();
